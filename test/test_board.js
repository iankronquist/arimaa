...
describe('Board', function() {
  ...
  describe('fromGame', function() {
    ...
    describe('game 1000 tests', function() {
      ...
      it('should have the right piece codes', function() {
        expect(_.pluck(game1000.moves[2].steps, 'pieceCode')).to.deep.equal(['H', 'M', 'R']);
        expect(_.pluck(game1000.moves[3].steps, 'pieceCode')).to.deep.equal(['d', 'h', 'r', 'r', 'r']);
        expect(_.pluck(game1000.moves[4].steps, 'pieceCode')).to.deep.equal(['H', 'H', 'E', 'R']);
        expect(_.pluck(game1000.moves[5].steps, 'pieceCode')).to.deep.equal(['d', 'd', 'd', 'm', 'r']);
      });

      it('should have the right starting positions', function() {
        expect(_.pluck(game1000.moves[2].steps, 'from')).to.deep.equal(['g2', 'd2', 'h2']);
        expect(_.pluck(game1000.moves[3].steps, 'from')).to.deep.equal(['b7', 'a7', 'f7', 'f6', 'c7']);
        expect(_.pluck(game1000.moves[4].steps, 'from')).to.deep.equal(['g3', 'b2', 'e2', 'h3']);
        expect(_.pluck(game1000.moves[5].steps, 'from')).to.deep.equal(['g7', 'g6', 'f6', 'd7', 'c6']);
      });

      it('should have the right directions', function() {
        expect(_.pluck(game1000.moves[2].steps, 'direction')).to.deep.equal(['n', 'n', 'n']);
        expect(_.pluck(game1000.moves[3].steps, 'direction')).to.deep.equal(['s', 's', 's', 'x', 's']);
        expect(_.pluck(game1000.moves[4].steps, 'direction')).to.deep.equal(['s', 'n', 'n', 'n']);
        expect(_.pluck(game1000.moves[5].steps, 'direction')).to.deep.equal(['s', 'w', 'x', 's', 's']);
      });

      it('should have the right destination positions', function() {
        expect(_.pluck(game1000.moves[2].steps, 'to')).to.deep.equal(['g3', 'd3', 'h3']);
        expect(_.pluck(game1000.moves[3].steps, 'to')).to.deep.equal(['b6', 'a6', 'f6', null, 'c6']);
        expect(_.pluck(game1000.moves[4].steps, 'to')).to.deep.equal(['g2', 'b3', 'e3', 'h4']);
        expect(_.pluck(game1000.moves[5].steps, 'to')).to.deep.equal(['g6', 'f6', null, 'd6', 'c5']);
      });
